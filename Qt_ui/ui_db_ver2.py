...
class SignUpClass(QDialog, Ui_joinForm):
    send_join_data_signal = pyqtSignal(list)
    def __init__(self):
        super().__init__()
    ...
    def check_info(self):
        ...
        if self.le_pw_input.text() != self.le_cpw_input.text():
            ...
            self.send_join_data_signal.emit(data_list)
            # temp_queue.put(data_list)
            self.show_message_box("can_sign_up")
            self.close()
            # 메인으로 보내지 않고 , 바로 서버로 보내버리면 되지 않을까/
            # main = MainClass()
...
class MainClass(QMainWindow, Ui_mainForm):
    ...
    def initSignal(self):
        ...
        self.msg_box_signal.connect(self.open_msg_box)
    ...
    def receive_thread(self):
        while True:
            ...
            self.process_get_data(json_obj)

    def process_get_data(self, get_data):

        try:
            header = get_data['header']
            self.user_id = get_data['user_id']
            if header == 'return_login':
                self.process_return_login(get_data)
        except:
            pass

    def process_return_login(self, data):
        return_data = data['return']
        if return_data == "not_registered_this_car_number":
            # 메시지 박스에 "가입되지않은 정보입니다." 출력
            self.msg_box_signal.emit(return_data)
            # QMessageBox.about(self, '에러 확인 요청', '가입되지 않은 정보입니다.')  # 병렬처리가 문제였다고 한다. -> 그럼 기존에도 터져야 하는게 아닌가? 통신하는 과정이 문제가 있는걸까.
            # 근데 병렬 문제가 맞았는듯. 시그널로 바꾸니까 그냥 되네...통신의 병렬과도 어떤 관계가 있는건가보다.
        elif return_data == "check_pw":
            self.msg_box_signal.emit(return_data)
        else:

            self.stackedWidget.setCurrentIndex(1)

    def open_msg_box(self, text):
        if text == "not_registered_this_car_number":
    ...
    def signup_dialog_act(self):
        ...
        sign_up_dialog_obj.send_join_data_signal.connect(self.get_other_class_data)
        sign_up_dialog_obj.exec_()

    def get_other_class_data(self, other_class_data):
        self.get_data_list = other_class_data
        send_data = {
            'header': 'membership_join',
            'car_plate_number': self.get_data_list[0],
            'password': self.get_data_list[1],
            'name': self.get_data_list[3],
            'contact': self.get_data_list[4]
        }
        self.send_fuc(send_data)
    ...
    def log_in_check(self):
        ...
        login_msg = {
            'header': 'login',
            'user_input_car_number': user_input_car_number,
            'user_input_pw': user_input_pw
        }

        self.send_fuc(login_msg)
